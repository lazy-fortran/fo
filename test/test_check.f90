...
contains
    ...
    subroutine test_fmt_check_files_limits_scope()
        character(len=512) :: project_dir, src_dir, output
        character(len=512) :: files(1)
        integer :: u, exitcode

        call make_tmp_path('fo_fmt_files_project', project_dir)
        src_dir = trim(project_dir)//'/src'
        call execute_command_line('mkdir -p '//trim(src_dir), wait=.true.)

        open (newunit=u, file=trim(project_dir)//'/fpm.toml', status='replace')
        write (u, '(a)') 'name = "fmt_files_project"'
        close (u)
        ...
        files(1) = trim(src_dir)//'/good.f90'
        call fo_fmt_check_files(project_dir, files, 1, output, exitcode)
        call assert(exitcode == 0, 'format file list ignores unlisted files')
        call assert(len_trim(output) == 0, &
            'format file list keeps clean output for selected files')

        call fo_fmt_check_run(project_dir, output, exitcode)
        call assert(exitcode /= 0, 'full format check still scans project')
